...
resource "aws_security_group" "backend_ec2" {
  ...
  description = "Allow traffic from Internal ALB to microservices"
  vpc_id      = var.vpc_id
...
resource "aws_security_group" "rds" {
  ...
  description = "Allow PostgreSQL traffic from Backend EC2"
  vpc_id      = var.vpc_id
...
resource "aws_iam_policy" "bedrock_invoke" {
  ...
  description = "Allow EC2 to invoke Bedrock models for AI descriptions and RAG"
  policy      = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
